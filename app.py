                ...
                if st.session_state.get("show_sources") and meta.get("chunks"):
                    with st.expander("🔍 View Retrieved Context"):
                        for i, (chunk, conf) in enumerate(
                            ...
                            chunk_srcs = meta.get("chunk_sources", meta.get("sources", []))
                            src = chunk_srcs[i] if i < len(chunk_srcs) else "?"
                            st.markdown(f"""
...
def main():
    ...
    if prefill:
        # A suggestion button was clicked: answer it immediately without
        # requiring the user to also press "Ask".
        question = prefill
        submitted = True
    else:
        ...
        question = user_input.strip() if submitted else ""
        ...
        if not is_index_ready(FAISS_INDEX_PATH):
            ...
            st.session_state.chat_history.append({
                ...
                "meta": {
                    ...
                    "sources"        : result["unique_sources"],  # for the summary line
                    "chunks"         : result["chunks"],
                    "chunk_sources"  : result["sources"],          # per-chunk, for the expander
